...
# --- FIX: Persist Matplotlib Cache (Works with Single File Exe) ---
def set_mpl_cache_dir():
    # 1. Check if running as a PyInstaller bundle (One-file or One-dir)
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        # In --onefile mode, _MEIPASS is the temp folder where the exe unpacks
...
# Run this BEFORE importing matplotlib
set_mpl_cache_dir()
# ------------------------------------------------------------------

# --- Library Imports ---

# PyQt5: For all GUI elements
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.series_colors = [] # to store the random palette
        self.series_markers = ['o', 'D', '^', 's', 'v', '*']
        # --- Build UI ---
        self._setup_ui()

    def _setup_ui(self):
        """Sets up the main UI layout and widgets."""
        self.setWindowTitle("JMB-Scripts MM-fit - v5.8-")
    ...
    def _create_widgets(self):
        ...
        self.recolor_button = QPushButton("Change Colors")
    ...
    def _create_layouts(self, main_layout):
        ...
        buttons_layout.addWidget(self.recolor_button)
    ...
    def _connect_signals(self):
        ...
        self.recolor_button.clicked.connect(self.change_colors)
    ...
    def _apply_styles(self):
        ...
        self.recolor_button.setStyleSheet(self.STYLE_SECONDARY)
    ...
    def paste_from_excel(self):
        ...
        # --- PRE-SCAN DATA FOR WARNINGS ---
        s_values_test = []
        for row in rows:
            vals = row.split('\t')
            if vals:
                try:
                    s_values_test.append(float(vals[0].replace(',', '.')))
                except ValueError:
                    pass
        
        # Warning 1: Missing [S]0 = 0
        if 0.0 not in s_values_test:
            reply = QMessageBox.question(self, "Missing 0,0 data", 
                                         "Check the origin point (0,0).\n\nDo you still want to proceed?",
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.No:
                return # Abort paste operation
                
        # Warning 2: Sorting Check
        if s_values_test and s_values_test != sorted(s_values_test):
            reply = QMessageBox.question(self, "Sorting Warning", 
                                         "Your Substrate concentrations ([S]0) are not sorted from smallest to biggest.\n\nDo you still want to proceed?",
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.No:
                return # Abort paste operation
        self.reset_data()
        ...
        # Generate a random, high-contrast palette
        high_contrast_colors = list(plt.cm.tab10.colors)
        random.shuffle(high_contrast_colors)
        self.series_colors = high_contrast_colors
        ...
        for i, row in enumerate(rows):
            ...
            for j, value in enumerate(values):
                ...
                self.data_table.setItem(i, j + 1, QTableWidgetItem(clean_val))

        self.data_table.resizeColumnsToContents()
        self._update_column_panel(headers)
    ...
    def change_colors(self):
        """Randomizes the high-contrast colors again and updates the plot."""
        if not self.series_colors:
            return # Do nothing if no data is loaded yet
            
        high_contrast_colors = list(plt.cm.tab10.colors)
        random.shuffle(high_contrast_colors)
        self.series_colors = high_contrast_colors
        
        # Automatically redraw the plot with the new colors
        self.fit_data()
    ...
    def show_lineweaver_burk(self):
        ...
        # Pass self.series_markers to the new window
        self.lb_window = LineweaverBurkWindow(self.processed_data_series_lb, self.excluded_data, self.data_table, self.Km_best, self.series_colors, self.series_markers, self)
        self.lb_window.show()
    ...
    def _plot_mm_series(self, ax, data, popt, stats, index, total):
            ...
            marker = self.series_markers[index % len(self.series_markers)] # <--- Get marker
            
            # Plot included data with the dynamic marker
            ax.scatter(data['s_fit'], data['v_fit'], label=f"Data {data['label']}", color=color, marker=marker, s=30)
            
            # Plot excluded data as 'x'
            if data['excluded_s'].size > 0:
                ax.scatter(data['excluded_s'], data['excluded_v'], color=color, marker='x', alpha=0.5, s=30)
    ...
    def _plot_residuals(self, subplot_spec, share_ax, data, popt, index, total):
        ...
        # Apply the marker here
        ax_res.scatter(data['s_fit'], residuals, color=color, marker=marker, s=30)
        
        if data['excluded_s'].size > 0:
            excluded_residuals = data['excluded_v'] - self.michaelis_menten(data['excluded_s'], *popt)
            ax_res.scatter(data['excluded_s'], excluded_residuals, color=color, marker='x', alpha=0.5, s=30)
...
class LineweaverBurkWindow(QMainWindow):
    ...
    def __init__(self, lb_series_data, excluded_data, data_table, km_best, series_colors, series_markers, parent=None):
        ...
        self.series_markers = series_markers # <--- Store the markers
        self._setup_ui()
    ...
    def _update_plot(self):
        ...
        for i, data in enumerate(self.lb_plot_data):
            ...
            marker = self.series_markers[i % len(self.series_markers)]
            
            # --- MUST HAVE THIS FOR AUTO-SCALING ---
            all_inv_s.extend(data['included_inv_s'])
            all_inv_s.extend(data['excluded_inv_s'])
            all_inv_v.extend(data['included_inv_v'])
            all_inv_v.extend(data['excluded_inv_v'])
            
            # Plot included points with dynamic marker, excluded as 'x'
            self.ax.scatter(data['included_inv_s'], data['included_inv_v'], label=f"Series {data['label']}", color=color, marker=marker, s=30)
            self.ax.scatter(data['excluded_inv_s'], data['excluded_inv_v'], color=color, alpha=0.5, marker='x', s=30)
...
class ExclusionDialog(QDialog):
    ...
    def _setup_ui(self, s_values, v_values, initial_exclusions):
        ...
        self.resize(650, 450) # Slightly wider to accommodate the new column
        ...
        # Remove spacing between widgets so the borders touch like a real table
        grid.setSpacing(0) 

        # --- ADD THIN GRID STYLING ---
        grid_widget.setStyleSheet("""
            QLabel { border: 1px solid #d3d3d3; padding: 4px; }
            QCheckBox { border: 1px solid #d3d3d3; padding: 4px; margin: 0px; }
        """)

        # Create column headers
        grid.addWidget(QLabel("<b>#</b>"), 0, 0, Qt.AlignmentFlag.AlignCenter) # New Row Number column
        grid.addWidget(QLabel("<b>[S]0</b>"), 0, 1, Qt.AlignmentFlag.AlignCenter)
        for v_idx in v_values:
            # Shifted column index by +1 (starts at 2 now)
            grid.addWidget(QLabel(f"<b>Series v{v_idx}</b>"), 0, 2 + v_idx * 2, 1, 2, Qt.AlignmentFlag.AlignCenter)

        # Create rows
        for row_idx, s_val in enumerate(s_values):
            # Add Row Number (1-based index)
            grid.addWidget(QLabel(str(row_idx + 1)), row_idx + 1, 0, Qt.AlignmentFlag.AlignCenter)
            
            # Add [S]0 value
            grid.addWidget(QLabel(f"{s_val:.2E}" if not np.isnan(s_val) else "N/A"), row_idx + 1, 1, Qt.AlignmentFlag.AlignCenter)
            ...
            for v_idx, v_list in v_values.items():
                ...
                # Shifted column indices by +1
                grid.addWidget(cb, row_idx + 1, 2 + v_idx * 2, Qt.AlignmentFlag.AlignCenter)
                grid.addWidget(QLabel(v_val_str), row_idx + 1, 3 + v_idx * 2, Qt.AlignmentFlag.AlignCenter)
        ...
        # We explicitly style the button so it doesn't inherit the thin grey borders from the grid
        apply_button.setStyleSheet(MainWindow.STYLE_ACTION)
